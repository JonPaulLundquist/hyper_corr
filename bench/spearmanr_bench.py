...
import scipy.stats
import math
from hyper_corr import spearmanr_ties, spearmanr_noties, spearmanr
...
print(f"{'N':>8} | {'SciPy(ms)':>10} | {'Hyper(ms)':>10} | {'Speed×':>8} | {'IQR':>8} | {'Δrho(max)':>10} | {'Δp(max)':>10}")
print("-"*89)

for n in sizes_order:

    scipy_times, numba_times, ratios = [], [], []
    max_d_rho, max_d_p = 0.0, 0.0
    ...
    scipy_fn0 = (lambda xs=xs0, ys=ys0: scipy.stats.spearmanr(xs, ys))
    if TIES is True:
        numba_fn0 = (lambda xs=xs0, ys=ys0, nn=nn0: spearmanr_ties(xs, ys, nn))
    elif TIES is False:
        numba_fn0 = (lambda xs=xs0, ys=ys0, nn=nn0: spearmanr_noties(xs, ys, nn))
    else:  # "unknown" -> always use unified spearmanr
        numba_fn0 = (lambda xs=xs0, ys=ys0: spearmanr(xs, ys))
    ...
    for k in range(BLOCKS):
        ...
        res_ref = scipy.stats.spearmanr(xs, ys)
        rho_ref, p_ref = float(res_ref.statistic), float(res_ref.pvalue)

        # zero-arg callables for this block
        scipy_fn = (lambda xs=xs, ys=ys: scipy.stats.spearmanr(xs, ys))
        if TIES is True:
            numba_fn = (lambda xs=xs, ys=ys, nn=nn: spearmanr_ties(xs, ys, nn))
        elif TIES is False:
            numba_fn = (lambda xs=xs, ys=ys, nn=nn: spearmanr_noties(xs, ys, nn))
        else:  # "unknown"
            numba_fn = (lambda xs=xs, ys=ys: spearmanr(xs, ys))

        # --- base deltas once per block (EXCLUDED from timing) ---
        rho_chk, p_chk = numba_fn()
        dt = abs(rho_ref - rho_chk)
        dp = abs(p_ref - p_chk)
        if dt > max_d_rho: max_d_rho = dt
    ...
    print(f"{n:8d} | {scipy_med*1e3:10.3f} | {numba_med*1e3:10.3f} | {speed_med:8.2f} | "
          f"{speed_iqr:8.2f} | {max_d_rho:10.3e} | {max_d_p:10.3e}")