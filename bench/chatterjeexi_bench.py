...
import scipy.stats
import math
from hyper_corr import chatterjeexi_ties, chatterjeexi_noties, chatterjeexi
...
print(f"{'N':>8} | {'SciPy(ms)':>10} | {'Hyper(ms)':>10} | {'Speed×':>8} | {'IQR':>8} | {'Δxi(max)':>10} | {'Δp(max)':>10}")
print("-"*89)

for n in sizes_order:

    scipy_times, numba_times, ratios = [], [], []
    max_d_xi, max_d_p = 0.0, 0.0
    ...
    if TIES is True or TIES is False:
        idx0 = np.argsort(x0, kind='quicksort')
        xs0, ys0 = x0[idx0], y0[idx0]
        nn0 = ys0.size

    scipy_fn0 = (lambda xs=x0, ys=y0: scipy.stats.chatterjeexi(xs, ys))
    if TIES is True:
        numba_fn0 = (lambda xs=xs0, ys=ys0, nn=nn0: chatterjeexi_ties(xs, ys, nn))
    elif TIES is False:
        numba_fn0 = (lambda xs=xs0, ys=ys0, nn=nn0: chatterjeexi_noties(xs, ys, nn))
        scipy_fn0 = (lambda xs=x0, ys=y0: scipy.stats.chatterjeexi(xs, ys, y_continuous=True))
    else:  # "unknown" -> always use unified chatterjeexi
        numba_fn0 = (lambda xs=x0, ys=y0: chatterjeexi(xs, ys))
    ...
    for k in range(BLOCKS):
        ...
        res_ref = scipy.stats.chatterjeexi(x, y)
        xi_ref, p_ref = float(res_ref.statistic), float(res_ref.pvalue)

        # zero-arg callables for this block
        scipy_fn = (lambda xs=x, ys=y: scipy.stats.chatterjeexi(xs, ys))
        if TIES is True:
            numba_fn = (lambda xs=xs, ys=ys, nn=nn: chatterjeexi_ties(xs, ys, nn))
        elif TIES is False:
            numba_fn = (lambda xs=xs, ys=ys, nn=nn: chatterjeexi_noties(xs, ys, nn))
            scipy_fn = (lambda xs=x, ys=y: scipy.stats.chatterjeexi(xs, ys, y_continuous=True))
        else:  # "unknown"
            numba_fn = (lambda xs=x, ys=y: chatterjeexi(xs, ys))

        # --- base deltas once per block (EXCLUDED from timing) ---
        xi_chk, p_chk = numba_fn()
        dt = abs(xi_ref - xi_chk)
        dp = abs(p_ref - p_chk)
        if dt > max_d_xi: max_d_xi = dt
    ...
    print(f"{n:8d} | {scipy_med*1e3:10.3f} | {numba_med*1e3:10.3f} | {speed_med:8.2f} | "
          f"{speed_iqr:8.2f} | {max_d_xi:10.3e} | {max_d_p:10.3e}")